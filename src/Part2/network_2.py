...
class MPLS_frame:
    ...
    def from_byte_S(self, byte_S):
        if byte_S[0] != '#':
            raise Exception('Not a MPLS frame.')
        return self(int(byte_S[1]), byte_S[2:])
    ...
    def to_byte_S(self):
        ...
        return "#" + str(self.label) + self.packet
...
class Router:
    ...
    def __init__(self, name, intf_cost_L, intf_capacity_L, rt_tbl_D, max_queue_size, mpls_table):
        self.stop = False #for thread termination
        self.name = name
